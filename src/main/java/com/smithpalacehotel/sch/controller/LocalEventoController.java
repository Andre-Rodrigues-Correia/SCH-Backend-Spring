...
import com.smithpalacehotel.sch.services.LocalEventoService;
...
public class LocalEventoController {
    ...
    public ResponseEntity<Collection<LocalEvento>> findAll() {
        Collection<LocalEvento> collection = service.findAll();
        return ResponseEntity.ok().body(collection);
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    public ResponseEntity<LocalEvento> find(@PathVariable Integer id) {
        LocalEvento obj = service.findById(id);
        return ResponseEntity.ok().body(obj);
    }

    @RequestMapping(method = RequestMethod.POST)
    public ResponseEntity<LocalEvento> insert(@Valid @RequestBody LocalEvento obj, BindingResult br) {
        if (br.hasErrors())
